...
#define NGX_HTTP_STREAM_SERVER_TRAFFIC_STATUS_DEFAULT_DUMP_PERIOD 60
...
#define ngx_http_stream_server_traffic_status_group_to_string(n)               \
  (u_char *)((n > 3)                                                           \
...
#define ngx_http_stream_server_traffic_status_max_integer                      \
  (NGX_ATOMIC_T_LEN < 12) ? "4294967295" : "18446744073709551615"

#define ngx_http_stream_server_traffic_status_boolean_to_string(b)             \
  (b) ? "true" : "false"

#define ngx_http_stream_server_traffic_status_triangle(n)                      \
  (unsigned)(n * (n + 1) / 2)

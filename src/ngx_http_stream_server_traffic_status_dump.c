...
static u_char  NGX_HTTP_STREAM_SERVER_TRAFFIC_STATUS_DUMP_DATA_PAD[] = { 0x53, 0x54, 0x56 };


static ssize_t ngx_http_stream_server_traffic_status_dump_header_read(ngx_file_t *file,
    ngx_http_stream_server_traffic_status_dump_header_t *file_header);
static ssize_t ngx_http_stream_server_traffic_status_dump_header_write(ngx_event_t *ev,
    ngx_file_t *file);
static void ngx_http_stream_server_traffic_status_dump_node_write(ngx_event_t *ev,
    ngx_file_t *file, ngx_rbtree_node_t *node);
static ngx_int_t ngx_http_stream_server_traffic_status_dump_update_valid(ngx_event_t *ev);
static ngx_int_t ngx_http_stream_server_traffic_status_dump_restore_add_node(ngx_event_t *ev,
     ngx_http_stream_server_traffic_status_node_t *ovtsn, ngx_str_t *key);


void
ngx_http_stream_server_traffic_status_file_lock(ngx_file_t *file)
{
...
void
ngx_http_stream_server_traffic_status_file_unlock(ngx_file_t *file)
{
...
void
ngx_http_stream_server_traffic_status_file_close(ngx_file_t *file)
{
...
static ssize_t
ngx_http_stream_server_traffic_status_dump_header_read(ngx_file_t *file,
    ngx_http_stream_server_traffic_status_dump_header_t *file_header)
{
    ssize_t  n;
    ...
    n = ngx_read_file(file, (u_char *) file_header,
                      sizeof(ngx_http_stream_server_traffic_status_dump_header_t), 0);
...
static ssize_t
ngx_http_stream_server_traffic_status_dump_header_write(ngx_event_t *ev, ngx_file_t *file)
{
    size_t                                        len;
    ssize_t                                       n;
    u_char                                       *p;
    ...
    ngx_http_stream_server_traffic_status_dump_header_t   file_header;
    ngx_log_error(NGX_LOG_INFO, ev->log, 0, "http sts status dump_header_write");
    ctx = ev->data;

    ngx_memzero(&file_header, sizeof(ngx_http_stream_server_traffic_status_dump_header_t));

    len = (ctx->shm_name.len >= NGX_HTTP_STREAM_SERVER_TRAFFIC_STATUS_DUMP_HEADER_NAME_SIZE)
          ? NGX_HTTP_STREAM_SERVER_TRAFFIC_STATUS_DUMP_HEADER_NAME_SIZE - 1
          : ctx->shm_name.len;

    p = file_header.name;
    p = ngx_cpymem(p, ctx->shm_name.data, len);
    file_header.time = ngx_http_stream_server_traffic_status_current_msec();
    file_header.version = nginx_version;

    n = ngx_write_fd(file->fd, &file_header, sizeof(ngx_http_stream_server_traffic_status_dump_header_t));
...
static void
ngx_http_stream_server_traffic_status_dump_node_write(ngx_event_t *ev, ngx_file_t *file,
    ngx_rbtree_node_t *node)
{
    ngx_http_stream_server_traffic_status_ctx_t   *ctx;
    ngx_http_stream_server_traffic_status_node_t  *vtsn;
    ngx_log_error(NGX_LOG_INFO, ev->log, 0, "http sts status dump_node_write");
    ctx = ev->data;

    if (node != ctx->rbtree->sentinel) {
        vtsn = (ngx_http_stream_server_traffic_status_node_t *) &node->color;

        (void) ngx_write_fd(file->fd, vtsn, sizeof(ngx_http_stream_server_traffic_status_node_t));
        (void) ngx_write_fd(file->fd, vtsn->data, vtsn->len);
        (void) ngx_write_fd(file->fd, NGX_HTTP_STREAM_SERVER_TRAFFIC_STATUS_DUMP_DATA_PAD,
                            sizeof(NGX_HTTP_STREAM_SERVER_TRAFFIC_STATUS_DUMP_DATA_PAD));

        ngx_http_stream_server_traffic_status_dump_node_write(ev, file, node->left);
        ngx_http_stream_server_traffic_status_dump_node_write(ev, file, node->right);
    }
}


static ngx_int_t
ngx_http_stream_server_traffic_status_dump_update_valid(ngx_event_t *ev)
{
    size_t                                        len;
    ssize_t                                       n;
    ngx_fd_t                                      fd;
    ...
    ngx_http_stream_server_traffic_status_ctx_t          *ctx;
    ngx_http_stream_server_traffic_status_dump_header_t   file_header;
    ngx_log_error(NGX_LOG_INFO, ev->log, 0, "http sts status dump_valid");
    ctx = ev->data;
    ...
    ngx_log_error(NGX_LOG_INFO, ev->log, 0, "http sts status dump_valid: len:%z",len);
    ...
    ngx_log_error(NGX_LOG_INFO, ev->log, 0, "http sts status dump_valid: current_msec:%z",current_msec);
    rc = ((current_msec - file_header.time) > ctx->dump_period) ? NGX_OK : NGX_ERROR;
    ngx_log_error(NGX_LOG_INFO, ev->log, 0, "http sts status dump_valid: rc:%z",rc);
    return rc;
}


ngx_int_t
ngx_http_stream_server_traffic_status_dump_execute(ngx_event_t *ev)
{
    u_char                               *name;
    ssize_t                               n;
    ngx_fd_t                              fd;
    ngx_file_t                            file;
    ngx_http_stream_server_traffic_status_ctx_t  *ctx;
    ngx_log_error(NGX_LOG_INFO, ev->log, 0, "http sts status dump_execute");
    ctx = ev->data;
    ...
    ngx_http_stream_server_traffic_status_file_lock(&file);
    ...
    ngx_http_stream_server_traffic_status_dump_node_write(ev, &file, ctx->rbtree->root);
...
static ngx_int_t
ngx_http_stream_server_traffic_status_dump_restore_add_node(ngx_event_t *ev,
     ngx_http_stream_server_traffic_status_node_t *ovtsn, ngx_str_t *key)
{
    size_t                                 size;
    uint32_t                               hash;
    ...
    ngx_http_stream_server_traffic_status_ctx_t   *ctx;
    ngx_http_stream_server_traffic_status_node_t  *vtsn;
    ngx_log_error(NGX_LOG_INFO, ev->log, 0, "http sts status dump_restore_add_node");
    ctx = ev->data;
...
void
ngx_http_stream_server_traffic_status_dump_restore(ngx_event_t *ev)
{
    off_t                                         offset;
    size_t                                        len;
    ssize_t                                       n;
    ...
    ctx = ev->data;
    ngx_log_error(NGX_LOG_INFO, ev->log, 0, "http sts status dump_restore: %s", ctx->dump_file.data);
    fd = ngx_open_file(ctx->dump_file.data, NGX_FILE_RDONLY, NGX_FILE_OPEN, 0);
    if (fd == NGX_INVALID_FILE) {
      //        ngx_log_debug1(NGX_LOG_DEBUG_HTTP, ev->log, ngx_errno,
        ngx_log_error(NGX_LOG_INFO, ev->log, ngx_errno,
    ...
    for ( ;; ) {
        ngx_memzero(buf, NGX_HTTP_STREAM_SERVER_TRAFFIC_STATUS_DUMP_DATA_BUF_SIZE);

        /* read: node */
        n = ngx_read_file(&file, (u_char *) &vtsn,
                          sizeof(ngx_http_stream_server_traffic_status_node_t), offset);
	  ...
	  //ngx_log_debug1(NGX_LOG_DEBUG_HTTP, ev->log, 0,
        ...
        if (vtsn.len > NGX_HTTP_STREAM_SERVER_TRAFFIC_STATUS_DUMP_DATA_BUF_SIZE) {
            offset += vtsn.len + sizeof(NGX_HTTP_STREAM_SERVER_TRAFFIC_STATUS_DUMP_DATA_PAD);
            continue;
        }
